...
/**
 * @Description: 1、面试题：String为什么是不可变的？
 *                  我看过源代码，String类有一个byte[]数组，这个byte[]数组采用了final修饰
 *                  因为数组一旦创建长度不可变。并且被final修饰的引用一旦指向某个对象之后，不可再
 *                  指向其它对象，所以String是不可变的！
 *                      "abc" 无法变成 "abcd"
 *               2、StringBuilder/StringBuffer为什么是不可变的？
